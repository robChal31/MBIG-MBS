...
function sendEmail($email, $name, $subject, $message, $config, $fileUrl, $cc = []) {
    ...
    if(mysqli_affected_rows($conn) == 1){
        ...
        $sql = "SELECT 
                    ...
                    leader.leadid, IFNULL(sc.name, db.school_name) as school_name, db.program, ec.sa_email, leader.generalname as approver_name
    ...
    if($status == 1) {
        ...
        }else if(($leadid == 16 || is_null($leadid)) && $id_user != 16){
        ...
        sendEmail($email, $ecname, $subject, $message, $config, $fileUrl, $cc);
        $sql = "UPDATE draft_benefit set status = 2 where id_draft = '$id_draft';";
        mysqli_query($conn, $sql);
