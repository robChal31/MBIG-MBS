                ...
                <div class="row g-4">
                    <div class="col-sm-12 col-xl-6">
                        <div class="bg-light rounded h-100 p-4">
                            <form method="POST" action="new-benefit-data-input-action.php" enctype="multipart/form-data">
                            ...
                            <div class="form-floating mb-3">
                                <select class="form-select" data-role="select" id="id_template_benefit" name="id_template_benefit" placeholder="Nama Benefit"
                                    aria-label="Nama Benefit">
                                    <option disabled hidden value="">Pilih Sekolah - Tahun</option>
                                    <?php
                                        $sql="select * from op_template_benefit";
                                        if($result = mysqli_query($conn,$sql))
                                        {
                                            while($row = mysqli_fetch_assoc($result))
                                            {
                                                echo "<option value='".$row['id_template_benefit']."'"; 
                                                if($rowa['id_template_benefit']==$row['id_template_benefit'])
                                                {
                                                    echo " selected ";
                                                }
                                                echo ">".$row['subbenefit']." - ".$row['benefit_name']."</option>";
                                            }
                                        }
                                    ?>
                                </select>
